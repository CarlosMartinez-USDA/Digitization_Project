...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0" xmlns="http://www.loc.gov/MARC21/slim" xmlns:f="http://functions" xmlns:local="http://www.loc.org/namespace" xmlns:marc="http://www.loc.gov/MARC21/slim" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" exclude-result-prefixes="f local marc xd xlink xs xsi">
    ...
    <!-- Maintenance note: For each revision, change revision number digitization.xsl
	
	Revision 1.02 - Added xmlns="http://www.loc.gov/MARC21/slim to result-document processing instruction.  2024/12/05 cm3
	Revision 1.01 - Additional changes logged here 2024/12/05 cm3 
    ...
    <xd:doc id="root" scope="component"><xd:desc><xd:p>Root: wraps marc:records in the marc:collection document node</xd:p></xd:desc></xd:doc>
    <xsl:template match="/">
    ...
    <xsl:template match="marc:datafield[@tag = '945']" mode="#default">
        <xsl:for-each-group select="*" group-by="descendant::node()">
            <xsl:apply-templates select="../marc:subfield[1][@code = 'd' or @code = 'e' or @code = 'f' or @code = 'g' or @code = 'n']">
                <xsl:with-param name="subfieldCode" select="@*"/>
            </xsl:apply-templates>
        </xsl:for-each-group>
    </xsl:template>
