...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
        ...
        <marc:record>
            ...
            <xsl:copy-of xml:space="preserve" select="marc:datafield[@tag &lt; '945']"/>
                ...
                <marc:datafield tag="945" ind1=" " ind2=" ">
                    <xsl:apply-templates select="marc:subfield[@code='a']" mode="group-945">
                        <xsl:with-param name="subfieldCode" select="@code='a'"/>
                    </xsl:apply-templates>    
                    <xsl:apply-templates select="marc:subfield[@code='v']" mode="group-945">
                        <xsl:with-param name="subfieldCode" select="@code='v'"/>
                    </xsl:apply-templates>
                    <xsl:apply-templates select="marc:subfield[@code='p']" mode="group-945">
                        <xsl:with-param name="subfieldCode" select="@code='p'"/>
                    </xsl:apply-templates> 
                </marc:datafield>
            <xsl:if test="marc:datafield[@tag = '945']/marc:subfield[1][@code='d'  or @code='e' or  @code='f' or @code='g' or  @code='n']">
                <marc:datafield tag="945" ind1=" " ind2=" "><xsl:for-each-group select="*" group-by="descendant::node()">
    ...
    <xsl:template match="marc:datafield[@tag = '945']" mode="group-945">
        <marc:datafield tag="945" ind1=" " ind2=" ">
            <xsl:choose>
                <xsl:when test="../marc:subfield[@code='a']='DigiMet'">
                    <marc:subfield code="a">DigiMet</marc:subfield>
                    <xsl:apply-templates select="marc:subfield[@code='v' or @code='p']" mode="group-945">
                        <xsl:with-param name="subfieldCode" select="@*"/>
                    </xsl:apply-templates>                        
                </xsl:when>
                <xsl:when test="marc:datafield[@tag = '945']/marc:subfield[@code='a']!='DigiMet'">
                    <xsl:copy-of select="marc:datafield[@tag = '945']/marc:subfield[@code='a']"/>
                    <xsl:apply-templates select="marc:datafield[@tag = '945']/marc:subfield[@code='p']" mode="group-945"/>                            
                    <xsl:apply-templates select="marc:datafield[@tag = '945']/marc:subfield[@code='v']" mode="group-945"/>                     
                </xsl:when>
                <xsl:otherwise>
                    <marc:subfield code="a">DigiMet</marc:subfield>
                    <xsl:apply-templates select="marc:datafield[@tag = '945']/marc:subfield[@code='d'  or @code='e' or  @code='f' or @code='g' or  @code='n']" mode="group-945"/>
                </xsl:otherwise>
            </xsl:choose>
        </marc:datafield>
    ...
    <xsl:template match="//marc:subfield[@code]" mode="subfield">
        ...
        <xsl:for-each select="$newNode"> <!--group-by="parent::*[position()]">-->            
