...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:template match="marc:record">
        <marc:record>
            ...
            <marc:datafield tag="945" ind1=" " ind2=" ">
                <xsl:apply-templates select="marc:datafield[@tag = '945']" mode="rewrap945-legacy"/>
            </marc:datafield>
            </xsl:if>
            <marc:datafield  tag="945" ind1=" " ind2=" "> 
                <xsl:apply-templates select="marc:datafield[@tag='945']" mode="rewrap945-digimet"/>
            </marc:datafield>           
            <xsl:copy-of xml:space="preserve" select="marc:datafield[@tag > '945']"/>
        </marc:record>            
    </xsl:template>

    <xsl:template match="marc:datafield[@tag='945']/marc:subfield[@code]" mode="rewrap945-legacy">
        <xsl:copy>
            <!-- Unwrap the child node -->
            <xsl:attribute name="code" select="replace(@code,'(a|v|p)', '$1')"/>
            <xsl:apply-templates select="text()" mode="unwrap945"/>
        </xsl:copy>
    </xsl:template>
    
    <!-- Template to match the parent node -->
    <xsl:template match="marc:datafield[@tag='945']/marc:subfield[@code]" mode="rewrap945-digimet">
        <xsl:copy>
            <!-- Unwrap the child node -->
            <xsl:attribute name="code" select="replace(@code,'(a|v|p)', '$1')"/>
            <xsl:apply-templates select="text()" mode="unwrap945"/>
    ...
    <xsl:template match="marc:subfield[@code]" mode="unwrap945">
        <xsl:copy>
