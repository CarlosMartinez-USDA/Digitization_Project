...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:template match="marc:record">
        <marc:record>
            ...
            <xsl:choose>
                <xsl:when test="count(marc:datafield[@tag = '945']/marc:subfield[@code]) &lt; 3">
                    <marc:datafield tag="945" ind1=" " ind2=" ">
                ...
                <xsl:otherwise>
                    <marc:datafield tag="945" ind1=" " ind2=" ">
                        <xsl:apply-templates select="marc:datafield[@tag = '945']" mode="group-945-subfield"/>
                    </marc:datafield>
                </xsl:otherwise>                
            </xsl:choose>
            <xsl:copy-of xml:space="preserve" select="marc:datafield[@tag > '945']"/>
    ...
    <xd:doc id="group" scope="component">
    ...
    <xsl:template match="marc:datafield[@tag = '945']" mode="group-945-subfield">
        <xsl:for-each-group select="../preceding-sibling::marc:subfield[@code &gt; 2]"  group-by="../following-sibling::marc:subfield[@code]">
            <xsl:apply-templates select="../marc:subfield[@code='a' or @code='p' or @code='v']" mode="subfield">
                <xsl:with-param name="subfieldCode" select="@*"/>
            </xsl:apply-templates>
        </xsl:for-each-group>
