...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0"
    xmlns:err="http://www.w3.org/2005/xqt-errors" xmlns:f="http://functions"
    xmlns:info="info:lc/xmlns/codelist-v1" xmlns:isolang="http://isolanguages"
    xmlns:local="http://www.local.gov/namespace" xmlns:marc="http://www.loc.gov/MARC21/slim"
    ...
    <xsl:variable name="alpha">ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz-|.
        '~</xsl:variable>
    ...
    <xd:doc>
        <xd:desc>
            <xd:p>Strips punctuation from the last character of a field, this version accepts two
                parameters:</xd:p>
            <xd:p>$str: string to be striped</xd:p>
            <xd:p>$pcodes: optional punctuation to override the defualt, if empty uses
                defualts</xd:p>
        </xd:desc>
        <xd:param name="str"/>
    ...
    <xd:doc>
        <xd:desc>
            ...
            <xd:p>subfields, if empty a space is used</xd:p>
        </xd:desc>
        <xd:param name="codes"/>
        <xd:param name="delimiter"/>
        <xd:param name="datafield"/>
    ...
    <xsl:function name="f:subfieldSelectSpecial" xmlns:f="http://functions">
        <xsl:param name="datafield" as="node()"/>
        <xsl:param name="codes"/>
        <xsl:variable name="translatedCodes" select="normalize-space(replace($codes,'\p{Pi}(\w)\p{Pf}, ','$1'))"/>
        <!-- Selects and prints out datafield -->
        <xsl:choose>
        ...
        <xd:desc>
            ...
            <xd:p>
                <xd:ul>
                    ...
                    <xd:li>
                        <xd:i>3.<xd:ref name="info:marcCountry" type="function">info:marcCountry</xd:ref>
                        <xd:note name="f:decodeMARCCountry" type="function">Replaces <xd:ref name="f:decodeMARCCountry" type="function"/> to reference <xd:a href="https://www.loc.gov/standards/codelists/countries.xml"/></xd:note></xd:i>
                    </xd:li>
                    <xd:li>
    ...
    <xsl:function name="f:add-namespace-prefix" as="node()*" xmlns:f="http://functions">
        ...
        <xsl:for-each select="$elements">
            ...
            <xsl:choose>
                ...
                <xsl:element name="{concat($add-prefix, if ($add-prefix = '') then '' else ':', local-name($element))}" namespace="{$add-namespace}">
                    <xsl:sequence select=" ($element/@*, f:add-namespace-prefix($element/node(), $add-namespace, $add-prefix))"/>
                </xsl:element>
                </xsl:when>
                <xsl:when test="$element instance of document-node()"> 
                    <xsl:document>
                        <xsl:sequence select=" f:add-namespace-prefix( $element/node(), $add-namespace, $add-prefix)" />
                     </xsl:document>
                </xsl:when>
    ...
    <xd:doc id="f:subjCatCode" scope="component">
        ...
        <xd:param name="subjCode">Convert subject category codes (MARC 072$a) into NAL Subject
            Categories.</xd:param>
    </xd:doc>
    ...
    <xd:doc id="f:decodeMARCCountry" scope="component">
        ...
        <xd:variable name="MARCcode">variable declared from the stylesheet.</xd:variable>
    ...
    <xd:doc id="info:marcCountry" scope="component">
        <xd:desc>
            ...
            <xd:p><xd:b>Purpose:</xd:b>Uses LC's countries.xml as lookup file to match MARC Country
                Code</xd:p>
            <xd:p><xd:b>Returns:</xd:b> the proper state or country name from the
                controlfield[@tag='008']</xd:p>
        </xd:desc>
        <xd:b>parameters &amp; variables</xd:b>
        <xd:param name="controlField008-15-17">marcCountry code variable passes the two or three
            letter valued to the $marcCode parameter</xd:param>
    ...
    <xsl:function name="info:marcCountry" as="xs:string" xmlns="info:lc/xmlns/codelist-v1">
        <xsl:param name="controlField008-15-17" as="xs:string"/>
        <!-- control field 008, position 15-17-->
        <xsl:variable name="country_name">
            <xsl:variable name="nodes">
                <xsl:copy-of select="document('http://www.loc.gov/standards/codelists/countries.xml')"/>
            </xsl:variable>
            <xsl:choose>
                <xsl:when test="$controlField008-15-17 = ''"/>
    ...
    <xsl:function name="info:langCode" as="xs:string" xmlns="info:lc/xmlns/codelist-v1">
        ...
        <xsl:variable name="langVar">
            <xsl:variable name="nodes">
                <xsl:copy-of select="document('http://www.loc.gov/standards/codelists/languages.xml')"/>
            </xsl:variable>
            <xsl:choose>
    ...
    <xd:doc>
        <xd:desc>
            ...
            <xd:p><xd:b>Purpose: </xd:b>The f:capitalize-first function capitalizes the first character of $arg. If the first character is not a lowercase letter, $arg is left unchanged. It capitalizes only the first character of the entire string, not the first letter of every word<xd:i>f:capitalize-first</xd:i></xd:p>
        </xd:desc>
        <xd:param name="arg"/>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>
            ...
            <xd:p><xd:b>Purpose:</xd:b>The f:sentence-case function treats a string like two parts; the first character of $arg is capitalized, the rest of the string is lowercase. (e.g. "STRING THEORY" ---> "String theory"). Thus, even if the second letter of $arg is not a lowercase letter, the rest of the string is still lowercase. </xd:p> 
        </xd:desc>
        <xd:param name="arg"/>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>
            ...
            <xd:p><xd:b>Purpose: </xd:b>The f:proper-case function performs a "choose when" test first determining if a string contains two substrings, when it does, it capitalizes the first character of each substring. when it does not, it capitalizes the first character of the substring, and the rest of the string is lowercase, making use of <xd:i>f:proper-case</xd:i>function in both instances</xd:p>
    ...
    <xsl:function name="f:proper-case" xmlns:f="http://functions">
        ...
        <xsl:sequence select=" if (contains($arg, $otherChars)) then concat(f:sentence-case($substring-1a), ($otherChars), (f:sentence-case($substring-1b))) else if (contains($arg, $white-space)) then concat(f:sentence-case($substring-2a), ($white-space), (f:sentence-case($substring-2b))) else f:sentence-case($arg)"/>     </xsl:function>
    ...
    <xd:doc>
        <xd:desc>
            ...
            <xd:p><xd:b>Purpose: </xd:b>The f:typeIdentifier xpath is passed to the $uri param. The  sequunce select uses the fn:matches($uri, $regex) tests the $arg. If $arg matches the $regex, then fn:replace($uri, $regex, $flag),</xd:p>
            <xd:p><xd:b>Returns: </xd:b> Removes the first and last substrings, leaving the org name.</xd:p>
            <xd:p>
                <xd:b>Parameters</xd:b>
            </xd:p>
        </xd:desc>
        <xd:param name="uri"/>
       <xd:variable name="id"><xd:p>uses fn:replace to break up the uri and determine the uncontrolled identifier informtion to put into the type attribute.</xd:p></xd:variable>
    </xd:doc>
    <xsl:function name="f:nameIdentifier" as="xs:string" xmlns:f="http://functions">
        ...
        <xsl:sequence select="if (matches($uri, '^https?://(www)?[id|agclass|lod|entities]+\.[a-z]+\.[a-z]+(\.[a-z]+)?.*')) then f:authority($id) else if ((contains($uri, 'orcid') or contains($uri, 'viaf') or contains($uri, 'isni') or matches($uri, '[a-z]+')) = true()) then substring-before($id, '.') else $uri"/>
    </xsl:function>
...
<!-- f:authority -->
    ...
    <xsl:function name="f:authority" as="xs:string">
        ...
        <xsl:sequence select=" if (contains($authority, 'loc')) then 'lcnaf' else if (contains($authority, 'nlm')) then 'mesh' else ''"/>
    </xsl:function>
    ...
    <xsl:function name="f:modsTotalPgs" xmlns:f="http://functions">
        ...
        <xsl:if test="(string(number($fpage)) != 'NaN' and string(number($lpage)) != 'NaN')">
            <total><xsl:value-of select="number($lpage) - number($fpage) + 1"/> </total>
        </xsl:if>
    </xsl:function>
    ...
    <xd:doc>
        <xd:desc>
            ...
            <xd:p><xd:b>Parameters: </xd:b>The $uris parameter is passed into a single $uri variable.</xd:p>
            <xd:p>If the $uri variable contains an opening bracket, the is substituted with its percent-encoded hex value.</xd:p>
            <xd:p>If the $sub1 variable contains closing bracket, it is substituted with it's hex value. </xd:p>
            <xd:p>Avoids invalidation errors when literal brackets are found within a URI. </xd:p>
        </xd:desc>
        <xd:param name="uris"/>
    </xd:doc>
    <xsl:function name="f:percentEncode" xmlns:f="http://functions">
        ...
        <xsl:sequence select=" if (contains($uri, '[') or (contains($uri, ']'))) then ($sub2) else $uri"/>
        </xsl:for-each>
    </xsl:function>
    ...
    <xsl:function name="f:punctuation-trim">
     ...
     <xsl:sequence select="if ($punctuated = '') then ($punctuated) else if (matches($punctuated, '(^.*)?[\[\]:;](.*$)?')) then (replace($punctuated, '(^.*)?[\[\]:;](.*$)?', '$1')) else $punctuation_removed"/>         </xsl:for-each>
    </xsl:function>
</xsl:stylesheet>
